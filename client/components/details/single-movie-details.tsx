...
function SingleMovie({ href }: { readonly href: string }) {
  ...
  // Check if movie is already in compare list
  const isInCompare = movie
    ? compareMovies.some((m) => m.id === movie.id)
    : false;
  const isCompareFull = compareMovies.length >= maxMovies;

  const handleCompareToggle = () => {
    if (!movie) return;

    if (isInCompare) {
      dispatch(removeFromCompare(movie.id));
    } else if (!isCompareFull) {
      const type = href.startsWith("tv") ? "tv" : "movie";
      const compareMovie: CompareMovie = {
        id: movie.id,
      ...
      dispatch(addToCompare(compareMovie));
    }
  };
      ...
      <section className="max-w-container px-6  text-white">
        ...
        <div className="details-banner opacity-[.99] md:flex gap-x-10 text-white h-full py-10 ">
          ...
          <div>
            ...
            <div className="pb-4 flex flex-wrap gap-3">
              <button
                ...
                className={`flex items-center gap-3 px-5 py-3 rounded-xl font-medium transition-all duration-300 ${
                  isInCompare
                    ...
                    : "bg-gradient-to-r from-pink to-orange text-white hover:opacity-90 hover:scale-[1.02]"
                }`}
              >
                {isInCompare ? (
                  <>
                  ...
                  <>
                    ...
                    <span>
                      {isCompareFull ? "Compare Full (3/3)" : "Add to Compare"}
                    </span>
                  </>
                )}
              </button>
              ...
              {movie && (
                <WishlistButton
                  ...
                  type={href.startsWith("tv") ? "tv" : "movie"}
                />
              )}
            </div>
            ...
            <div className="flex gap-x-8 py-6 border-b border-[rgba(255,255,255,.1)] flex-wrap gap-y-3">
